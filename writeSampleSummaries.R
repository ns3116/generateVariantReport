...
writeDNM <- function(dnm,rtf){
    for(i in 1:dim(dnm)[1]){
        ...
        if(dnm[i,]$Ctrl.MAF == 0 & dnm[i,]$Evs.All.Maf == 0 & dnm[i,]$ExAC.global.maf == 0){
...
writeCHET <- function(chet,rtf){
    for(i in 1:dim(chet)[1]){
        ...
        addText(rtf,"\n")
        if(chet[i,]$Ctrl.MAF.1 == 0 & chet[i,]$Evs.All.Maf.1 == 0 & chet[i,]$ExAC.global.maf.1 == 0){
        ...
        addText(rtf,"\n")
        if(chet[i,]$Ctrl.MAF.2 == 0 & chet[i,]$Evs.All.Maf.2 == 0 & chet[i,]$ExAC.global.maf.2 == 0){
            addText(rtf,"The second variant is absent from internal and external control samples. ")}
        else{addText(rtf,paste0("The first variant has a control MAF of ",chet[i,]$Ctrl.MAF.2*100,"% in IGM controls, ",chet[i,]$Evs.All.Maf.2*100,"% in EVS, and ",chet[i,]$ExAC.global.maf.2,"% in ExAC. "))}
