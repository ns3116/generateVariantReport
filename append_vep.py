...
parser.add_option("--appended_file",
                  dest="output_",
...
class Variant(object):
	...
	if '-' in self.position:
        ...
        for con in self.consequences[1:]:
            try:
                ...
                if not options.mute:
                    print 'Warning: encountered un-indexed consequence: %s' % (self.con)
                ...
                if var1.canonical and not var2.canonical:
                    ...
                    if var1.ccds and not var2.ccds:
                        ...
                        if var1.feature != var2.feature and var1.canonical and var2.canonical:
                            print 'Warning: tie could not be broken between canonical features %s and %s.'\
                                      % (var1.feature, var2.feature)
                            return var1
        return var2
    ...
    def __hash__(self):
	return hash(':'.join( [ self.chrom, str(self.position[0]), str(self.position[-1])  ] ) )
...
def readVEP(variants, atavids):
	...
	a = h.same_pos(variant)
	    ...
	    h = Variant.compare(h, variant)
            like.append(variant)
        if not a or i == len(variants) - 1:
            highest[ (h.chrom, h.position[0], h.position[-1]) ] = h
            n = sum(1 for v in like if h.sev_con in v.consequences
                    and h.gene == v.gene and v.feature_type == 'Transcript') + 1
            d = sum(1 for v in like if h.gene == v.gene and v.feature_type == 'Transcript') + 1
	    h.setfreq(n , d)
	    h = variant
	    like = [h]
    ...
    unmatched = [obj for obj in highest.values() if obj not in towrite]
    if len(unmatched) > 0:
	print "ERROR: vep output could not be matched with input variants"
	quit()
    ...
    for row in reader:
        ...
        try:
            for c in range(tar_cols):
                row.append(highest[i * tar_cols + c].hgvsc)
                row.append(highest[i * tar_cols + c].hgvsp)

                row.append(highest[i * tar_cols + c].freq)
        except IndexError:
            print 'Unexpected difference in sizes of vep and atav output.'
            exit(1)
        rows.append(row)
        i += 1
    reader.close()
...
def append_vep():
	...
	print "Error: Append file " + options.output_ + " already exists. Specify a different output file with --appended_file or overwrite existing file with --force_append."
	print
	quit()
    ...
    if options.vep:
	print " - Wrote stats to " + options.vep + "."
    print " - Done!"
